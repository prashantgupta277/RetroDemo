...
import './superadminedittemplate.css';
import HeaderComponent from './../../commonComponent/header';
import FooterComponent from './../../commonComponent/footer';
import { submit_get_Template, getEditCustomTemplateRes, submit_update_Template, updateEditCustomTemplateRes } from '../../actions/superAdminTemplateActions';
import { compose } from 'redux';
import { connect } from 'react-redux';
import { createStructuredSelector } from 'reselect';

class SuperAdminEditTemplate extends Component {
    constructor(props) {
        super(props);
        this.state = {
          editTemplateDetail: [],
        ...
        if (nextProps) {
          ...
          if (nextProps.editTemplateDetail) {
            if (nextProps.editTemplateDetail.user.superAdminTemplateReducer) {
              if (nextProps.editTemplateDetail.user.superAdminTemplateReducer.data && nextProps.editTemplateDetail.user.superAdminTemplateReducer.data.fetchSingleUsertemplate) {
                let tempInfo = nextProps.editTemplateDetail.user.superAdminTemplateReducer.data.fetchSingleUsertemplate
                this.setState({
          ...
          if (nextProps.updateTemplate) {
            if (nextProps.updateTemplate.user.superAdminTemplateReducer) {
              if (nextProps.updateTemplate.user.superAdminTemplateReducer.data && nextProps.updateTemplate.user.superAdminTemplateReducer.data.updateUserTemplate) {
                let tempInfoUpdate = nextProps.updateTemplate.user.superAdminTemplateReducer.data.updateUserTemplate
                this.setState({
  ...
  const mapStateToProps = createStructuredSelector({
    editTemplateDetail: getEditCustomTemplateRes,
    updateTemplate: updateEditCustomTemplateRes,
  });
  
  function mapDispatchToProps(dispatch) {
    return {
      handleFormSubmit: (data) => dispatch(submit_get_Template(data)),
      updateFormSubmit: (data) => dispatch(submit_update_Template(data))
    };
  }
  
  const withConnect = connect(mapStateToProps, mapDispatchToProps);
  
  export default compose(withConnect)(SuperAdminEditTemplate);